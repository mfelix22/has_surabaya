...
<body>
            ...
            <td class="info-section">
                <table>
                    ...
                    <tr>
                        <td>Bon Out(s)</td>
                        <td>:</td>
                        <td class="val">{{ $bonOutNumbers ?: '-' }} <small>({{ $completedBonOuts->count() }} completed)</small></td>
                    </tr>
                    <tr>
                        <td>Revenue</td>
                        <td>:</td>
    ...
    {{-- ===== MATERIAL COGS ===== --}}
    <div class="section-title">A. Direct Materials (COGS Material) – Aggregated from {{ $completedBonOuts->count() }} Bon Out(s)</div>
    ...
    @if ($unusedItems->count() > 0)
        <div style="padding: 8px; margin-bottom: 8px; background-color: #fff3cd; border: 1px solid #ffc107; border-radius: 3px;">
            <strong>⚠ Note:</strong> {{ $unusedItems->count() }} item(s) from Work Order BOM were NOT used in any Bon Out:
            <ul style="margin: 4px 0 0 20px; padding: 0;">
                @foreach ($unusedItems as $unused)
                    <li style="font-size: 10px;">
                        <strong>[{{ $unused->item->code }}]</strong> {{ $unused->item->name }} 
                        <span style="color: #666;">(Planned: {{ number_format($unused->demand_quantity, 2) }} {{ $unused->item->smallestUom->code ?? '' }})</span>
                    </li>
                @endforeach
            </ul>
        </div>
    ...
    @if (count($aggregatedItems) > 0)
        <table class="items-table">
            ...
            <tbody>
                @php $sumMaterial = 0; $rowNum = 1; @endphp
